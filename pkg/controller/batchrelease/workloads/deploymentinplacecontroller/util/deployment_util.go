...
func DeploymentPaused(deployment *apps.Deployment) bool {
	return strings.EqualFold(deployment.Annotations[util.DeploymentPausedAnnotation], "true")
}
...
func MaxUnavailable(deployment apps.Deployment) int32 {
	strategyByte, ok := deployment.Annotations[util.DeploymentStrategy]
	if !ok || strategyByte == "" {
		return 0
	}
	strategy := &apps.DeploymentStrategy{}
	if err := json.Unmarshal([]byte(strategyByte), strategy); err != nil {
		return 0
	}

	if !IsRollingUpdateStrategy(strategy) || *(deployment.Spec.Replicas) == 0 {
		return int32(0)
	}
...
// MaxUnavailableWithReplicas returns the maximum unavailable pods a rolling deployment can take.
func MaxUnavailableWithReplicas(deployment apps.Deployment, replicas int32) int32 {
	strategyByte, ok := deployment.Annotations[util.DeploymentStrategy]
	if !ok || strategyByte == "" {
		return 0
	}
	...
	// Error caught by validation
	_, maxUnavailable, _ := ResolveFenceposts(strategy.RollingUpdate.MaxSurge, strategy.RollingUpdate.MaxUnavailable, replicas)
	if maxUnavailable > *deployment.Spec.Replicas {
		return *deployment.Spec.Replicas
	}
	return maxUnavailable
...
func MaxSurge(deployment apps.Deployment) int32 {
	...
	if !IsRollingUpdateStrategy(strategy) {
		return int32(0)
	}
	// Error caught by validation
...
func IsRollingUpdate(deployment *apps.Deployment) bool {
	...
	return IsRollingUpdateStrategy(strategy)
}
...
func IsClaimedBy(deployment *apps.Deployment, release *appsv1alpha1.BatchRelease) bool {
	owner, ok := deployment.Annotations[util.BatchReleaseControlAnnotation]
	if !ok || owner == "" {
		return false
	}
	ownerRef := &metav1.OwnerReference{}
	if err := json.Unmarshal([]byte(owner), ownerRef); err != nil {
		return false
	}
	return ownerRef.UID == release.UID
}
