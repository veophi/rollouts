...
func (dc *DeploymentController) getNewReplicaSet(ctx context.Context, d *apps.Deployment, rsList, oldRSs []*apps.ReplicaSet, createIfNotExisted bool) (*apps.ReplicaSet, error) {
	...
	if existingNewRS != nil {
		...
		annotationsUpdated := util.SetNewReplicaSetAnnotations(d, rsCopy, newRevision, true, maxRevHistoryLengthInChars)
		...
		if util.HasProgressDeadline(d) && cond == nil {
			...
			condition := util.NewDeploymentCondition(apps.DeploymentProgressing, v1.ConditionTrue, util.FoundNewRSReason, msg)
	...
	newReplicasCount = integer.Int32Min(newReplicasCount, dc.currentBatchNewReplicaLimit(d))

	*(newRS.Spec.Replicas) = newReplicasCount
	// Set new replica set's annotation
	util.SetNewReplicaSetAnnotations(d, &newRS, newRevision, false, maxRevHistoryLengthInChars)
	...
	case err != nil:
		...
		if util.HasProgressDeadline(d) {
			cond := util.NewDeploymentCondition(apps.DeploymentProgressing, v1.ConditionFalse, util.FailedRSCreateReason, msg)
		...
		dc.EventRecorder.Eventf(d, v1.EventTypeWarning, util.FailedRSCreateReason, msg)
		return nil, err
	}
	...
	if !alreadyExists && util.HasProgressDeadline(d) {
		...
		condition := util.NewDeploymentCondition(apps.DeploymentProgressing, v1.ConditionTrue, util.NewReplicaSetReason, msg)
		util.SetDeploymentCondition(&d.Status, *condition)
		needsUpdate = true
	}
	if needsUpdate {
...
func (dc *DeploymentController) scale(ctx context.Context, deployment *apps.Deployment, newRS *apps.ReplicaSet, oldRSs []*apps.ReplicaSet) (bool, error) {
	...
	// rolling deployment.
	if util.IsRollingUpdate(deployment) || util.IsClaimedBy(deployment, dc.Release) {
		...
		for i := range allRSs {
			...
			if deploymentReplicasToAdd != 0 {
				proportion := util.GetProportion(rs, *deployment, deploymentReplicasToAdd, deploymentReplicasAdded)
...
func (dc *DeploymentController) cleanupDeployment(ctx context.Context, oldRSs []*apps.ReplicaSet, deployment *apps.Deployment) error {
	...
	for i := int32(0); i < diff; i++ {
		...
		if err := dc.Client.Delete(context.TODO(), rs); err != nil && !errors.IsNotFound(err) {
...
func calculateStatus(allRSs []*apps.ReplicaSet, newRS *apps.ReplicaSet, deployment *apps.Deployment) apps.DeploymentStatus {
	...
	if availableReplicas >= *(deployment.Spec.Replicas)-util.MaxUnavailable(*deployment) {
		minAvailability := util.NewDeploymentCondition(apps.DeploymentAvailable, v1.ConditionTrue, util.MinimumReplicasAvailable, "Deployment has minimum availability.")
		util.SetDeploymentCondition(&status, *minAvailability)
	} else {
		noMinAvailability := util.NewDeploymentCondition(apps.DeploymentAvailable, v1.ConditionFalse, util.MinimumReplicasUnavailable, "Deployment does not have minimum availability.")
