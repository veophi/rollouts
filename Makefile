...
deploy-config-gen: manifests kustomize manifests ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default > kruise-rollout-all-in-one.yaml
	echo -e "resources:\n- manager.yaml" > config/manager/kustomization.yaml
